...
public interface TextAdd{
    static String latexText(String text) {
        return "\\(" + text + "\\)";
    }

    static void textK1(ModelMap model, String[] arr) {
        model.addAttribute("textK1", "\\(\\begin{vmatrix} " + arr[0] + " & " + arr[1] + " \\\\ " + arr[1] + " & " + arr[2] + " \\end{vmatrix} \\)");
    }

    static String equation(double[] arr, String[] mults) {
        StringBuilder text = new StringBuilder(arr[0] + "x²");
        for (int i = 1;i<arr.length;i++)
            text.append(valueEqu(arr[i], mults[i]));
        return text.toString();
    }
    static void allEquation(ModelMap model, double[] arr) {
        String text = "Рівняння\\,поверхні:\\\\[0.1in]" + equation(arr, mults);
        model.addAttribute("equation", latexText(text));
    }

    static String posOrNeg(double number) {
        return (number < 0 ? "":"+") + number;
    }

    static String valueEqu(double coeff, String mult) {
        return posOrNeg(coeff) + mult;
    }

    static void gequation(ModelMap model, String text) {
        model.addAttribute("gequation", latexText(text));
    }

    static String nameValue(String name, double value) {
        return name + " = " +value + ",\\,";
    }

    static void coeff(ModelMap model, double[] arr){
    ...
    static void textI1(ModelMap model, double[] arr) {
    ...
    static void textI2(ModelMap model, double[] arr) {
    ...
    static void textI3(ModelMap model, double[] arr) {
    ...
    static void textK3(ModelMap model, double[] arr) {
